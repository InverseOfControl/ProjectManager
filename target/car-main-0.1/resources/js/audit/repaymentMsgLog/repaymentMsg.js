$(function() {
	...
	$('#repaymentMsg_result').datagrid({
		url : 'repaymentMsg/repaymentMsgList',
		fitColumns : true,
		border : false,
		pagination : true,
		pageSize : 20,
	 ...
	 // 设置分页控件
	   var p = $('#repaymentMsg_result').datagrid('getPager');
	    $(p).pagination({
	        pageList: [ 10, 20, 50 ]
	    });
...
function search(){
	var queryParams = $('#repaymentMsg_result').datagrid('options').queryParams;
	...
	queryParams.buildEndDate = $('#toolbar #buildEndDate').val();
	 setFirstPage("#repaymentMsg_result");
	$('#repaymentMsg_result').datagrid('options').queryParams = queryParams;
	$("#repaymentMsg_result").datagrid('reload');
}
