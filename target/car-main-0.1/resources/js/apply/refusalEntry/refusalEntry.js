...
$(function() {
    ...
    //营业网点
    $('#toolbar #salesDeptComb').combobox({
        url:'RefusalEntry/getCurSalesDept',
        valueField:'id',
        textField:'name',
        onLoadSuccess:function(){
            var data = $(this).combobox('getData');
    ...
    $('#toolbar #productComb').combobox({
        url:'RefusalEntry/getProductType',
        valueField:'id',
...
function showAttachmentDlg(loanId,productType,extensionTime){
	if(extensionTime==0)
	{
		if(productType == 1){
		    ...
		    window.open (rayUseUrl+"RefusalEntry/carImageUploadView/"+loanId, "newwindow","toolbar=yes,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,fullscreen=3");
...
function search(){	
    ...
    /*queryParams.requestStartDate = $('#toolbar #requestStartDate').datebox('getValue'); 
    queryParams.requestEndDate = $('#toolbar #requestEndDate').datebox('getValue'); */
    queryParams.requestStartDate = $('#toolbar #requestStartDate').val(); 
    queryParams.requestEndDate = $('#toolbar #requestEndDate').val(); 
				...
				if(loanDetailsVo.personType=='新客户'){
				   ...
				   //新增车贷 
					if(loanDetailsVo.productTypeId==2){
                        ...
                        var url = 'RefusalEntry/carLoanAdd';
                        $('#carLoanAdd').dialog({
                            title: '新增车贷拒单录入',
                            closed: false,  
                            cache: false,
...
;var commonApplyDataModule_plugin = function ($) {
    ...
    function loadSales (pageData,form) {
        ...
        //业务主任
        $(form).find('#bizName').combobox({
          url:'RefusalEntry/getBizDirectorByUser',
          valueField:'id',
          textField:'name',
          editable:false ,
          onBeforeLoad: function(param){                  
