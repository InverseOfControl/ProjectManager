...
function  contractSubmit(loanId){	
	parent.$.messager.confirm('确认','提交合同？', function(r) {
		...
		//如果是终止借款或流标，要重新生成合同并签订合同 add date 2018/2/5
		var lcbBorrowStatus = null;
		$.ajax({
			type : 'post',
			url : 'audit/contract/getLcbStatusById',
			data : {loanId:loanId},
			async : false,
			success : function(result) {
				lcbBorrowStatus = result.lcbBorrowStatus;
			}
		});
		if(lcbBorrowStatus != null && lcbBorrowStatus != '' && (lcbBorrowStatus == '10' || lcbBorrowStatus == '20')){
			parent.$.messager.show({
				title : '提示',
				msg : '请先重新生成合同！'
			});
