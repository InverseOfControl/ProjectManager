$().ready(function() {
	...
	$('#searchAreaBaseBut').click(function() {
		var queryParams = $('#baseAreaGrid').datagrid('options').queryParams;
		//网点名称
		...
		$('#baseAreaGrid').datagrid('options').queryParams = queryParams;
	    $("#baseAreaGrid").datagrid('reload');
	})
...
function selectedBaseArea(baseArea) {
	...
	$('#sysUserPanel').find('input[name="areaId"]').val(id);
	$('#sysUserPanel').find('input[name="fullName"]').val(name);
