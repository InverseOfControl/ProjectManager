...
Ext.define('Ext.ux.uploadPanel.UploadPanel',{
    ...
    columns : [
        ...
        {text: '自定义文件名', width: 200,dataIndex: 'fileName',editor: {xtype: 'textfield'}},
        ...
        {text: '进度', width: 130,dataIndex: 'percent',renderer:function(v){
            var stml =
                '<div>'+
                    ...
                    '<div style="float:left;background:#00FF00;width:'+v+'%;height:8px;"><div></div></div>'+
                    '</div>'+
        ...
        }},
        {
            ...
            width:50,
            items: [{
                icon: 'resources/css/swfupload/icons/delete.gif',
                tooltip: 'Remove',
    ...
    file_types : '*.jpg;*.jpeg;*.bmp;*.gif;*.png;*.zip;*.rar',
    file_types_description : '图片和压缩文件',
    file_upload_limit : 0,
    file_queue_limit : 100,
    post_params : {},
    upload_url : 'test.do',
    flash_url : "resources/js/swfupload/swfupload.swf",
    flash9_url : "resources/js/swfupload/swfupload_fp9.swf",
            ...
            items: [
                {
                    ...
                    text : "支持jpg、jpeg、bmp、gif、png、zip、rar类型文件上传。每次批量限"+fileQueueLimit+"个，单个文件不大于"+Math.floor(fileSizeLimit)+"M。",
                    disabled : true
                }
            ]
        }];
    ...
    upload_start_handler : function(file){
        ...
        this.addFileParam ( rec.get('id'), "fOName", rec.get('fileName'));
        this.addFileParam ( rec.get('id'), "combo_type", Ext.getCmp('combo_type').value );
        if(Ext.getCmp('subCombo_type')!=undefined)
    	{
        	this.addFileParam ( rec.get('id'), "subCombo_type", Ext.getCmp('subCombo_type').value );
    	}
       // this.addFileParam ( rec.get('id'), "comboTextField", Ext.getCmp('comboTextField').value );

        this.addFileParam ( rec.get('id'), "loanId", loanId ); //借款人编号
    ...
    upload_error_handler : function(file, errorCode, message){
        /*Ext.Msg.alert('提示' ,'上传失败.请更换IE7以上版本<br> 错误代码:' + errorCode);*/
        Ext.Msg.alert('提示' ,'上传失败，请联系管理员<br> 错误代码:' + errorCode);
        var me = this.settings.custom_settings.scope_handler;
        var rec = me.store.getById(file.id);
        rec.set('percent', 0);
        rec.set('status', file.filestatus);
    ...
    onUpload : function(){
        if(Ext.getCmp('combo_type').value==null ){
            Ext.Msg.alert('提示','请选择资料分类');
        }
        else if(Ext.getCmp('subCombo_type')!=undefined &&Ext.getCmp('subCombo_type').value==null)
    	{
            Ext.Msg.alert('提示','请选择资料二级分类');
    	}	
        else{
