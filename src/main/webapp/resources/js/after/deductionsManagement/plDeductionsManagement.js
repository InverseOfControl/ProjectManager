$(function() {
	...
	$('#exportExcelBt').linkbutton("disable");//plSendOfferBt
	/*$('#plSendOfferBt').linkbutton("disable");//plCannelOfferBt
	$('#plCannelOfferBt').linkbutton("disable");*/
	$('#export').hide();
	$('#editMoney').hide();
	$('#guarantee').hide();
	$('#changeTpp').hide();
	...
	if (userType == 6) {//小企业贷款可以担保代扣 修改金额
		...
		$("#toEditMoney").bind('click', toEditMoney);
		$('#plSendOffer').show();
		$('#plSendOfferBt').linkbutton("enable");
		//批量实时划扣
		$("#plSendOfferBt").bind('click', plSendOffer);	
		//plCannelOfferBt
		$('#plCannelOffer').show();
		$('#plCannelOfferBt').linkbutton("enable");
		//批量划扣取消
		$("#plCannelOfferBt").bind('click', plCannelOffer);	
	...
	$('#list_result').datagrid({
		...
		queryParams: {
			...
			cityId:$('#toolbar #cityComb').combobox('getValue')
		},
			...
			if (data != null) {
				if (data.rows.length > 0) {
					...
					for (var j = 0; j < data.rows.length; j++) {
						...
						if(!(data.rows[j].offerType==1&&data.rows[j].status==10&&hasIfEditCarLoan)){
	...
	//   界面生成
	$("#plImmediateBt").bind('click', windowOpen);
	// 生成划扣记录  提交按钮
...
function search() {
	 var opts = $("#list_result").datagrid("options");
	 opts.url = "after/deductionsManagement/getDeductionsManagementPage?plFlag=pl";
...
function windowOpen(){
	 window.open(rayUseUrl+"after/deductionsManagement/plListImmediately" 
			 ,"","toolbar=yes,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,fullscreen=3");
...
function changeOfferTpp(){
	...
	var length=rows.length;
	var ids = "";
	var tppType=$('#tppType').combobox('getValue');
	...
	if (rows.length > 0) {
		parent.$.messager.confirm('确认', '确定发送实时报盘?', function(r) {
			if (r) {
				...
				$.ajax({
					url : 'after/deductionsManagement/plToSendTpp',
					type : "POST",
					data : {
						offerId : rows[i].id,
					},
	...
	if (rows.length > 0) {
		parent.$.messager.confirm('确认', '确定取消实时报盘?', function(r) {
			if (r) {
				for (var i = 0; i < rows.length; i++) {
				$.ajax({
					url : 'after/deductionsManagement/plCannelCurrentOffer',
					type : "POST",
					data : {
						id : rows[i].id,
					},
